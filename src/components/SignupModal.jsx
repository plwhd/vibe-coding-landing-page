      ...
      <div className="mx-auto my-8 max-w-3xl overflow-hidden rounded-[2rem] bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-6 border-b border-slate-100 p-6">
          ...
          <button
            ...
            aria-label="关闭报名弹窗"
            onClick={onClose}
            className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-slate-200 text-slate-500 transition hover:border-qblue hover:text-qblue"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <form className="grid gap-5 p-6" onSubmit={(event) => event.preventDefault()}>
          ...
          <div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {['组队投递报名', '单人投递报名', '先单人报名，之后再确定'].map((item) => (
                <label key={item} className="flex cursor-pointer gap-2 rounded-2xl border border-slate-200 p-4 text-sm text-slate-600 hover:border-qblue">
                  <input type="radio" name="signupType" className="mt-1" />
                  {item}
                </label>
              ))}
            </div>
            <p className="mt-2 text-xs text-slate-500">友情提醒：议题涉及前后端配合，组队报名胜率更高。</p>
          </div>
          ...
          <div className="rounded-3xl bg-blue-50 p-5 text-sm leading-7 text-slate-600">
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="min-h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm outline-none transition focus:border-qblue focus:ring-4 focus:ring-blue-100"
      />
    </label>
  )
}
...
function Select({ label, options }) {
  return (
    <label className="block">
      <span className="mb-2 block text-sm font-semibold text-ink">{label}</span>
      <select className="min-h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm outline-none transition focus:border-qblue focus:ring-4 focus:ring-blue-100">
        <option>请选择</option>
        {options.map((option) => (
