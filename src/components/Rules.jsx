...
export default function Rules() {
  return (
    <section id="rules" className="section-pad bg-warm">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
          <Reveal>
            <div className="rounded-[28px] border border-warmline bg-white p-6">
              ...
              <div className="space-y-4">
                {rules.review.map((item) => (
                  <div key={item.title} className="rounded-[24px] bg-warm p-5">
                    ...
                    <p className="text-sm leading-7 text-olive">{item.detail}</p>
                    <div className="mt-4 h-2 overflow-hidden rounded-2xl bg-sand">
                      <div className="h-full rounded-2xl bg-qblue" style={{ width: item.weight }} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-[28px] border border-warmline bg-white p-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {rules.submission.map((item) => (
                  <div key={item} className="flex gap-3 rounded-[24px] bg-warm p-5">
                    <CheckCircle2 className="mt-1 h-5 w-5 shrink-0 text-mint" />
                    <p className="text-sm leading-7 text-olive">{item}</p>
                  </div>
                ))}
              </div>

              <details className="mt-5 rounded-[24px] border border-warmline bg-white p-5">
